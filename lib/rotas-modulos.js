...
async function handleModulos(ctx) {
      ...
      data = {
        ...
        threads: Object.values(CONTATOS_GABINETE).map((c) => ({
          ...
          mensagens: [{
            ...
            de: 'secretaria',
            autor: c.nome,
            texto: `Bom dia. Estou online no painel da ${c.cargo}. Pode falar por aqui.`,
            criadoEm: agora,
          }],
        })),
      };
  ...
  if (pathname === '/api/gabinete/chat' && req.method === 'POST') {
    ...
    // Resposta rápida do responsável (sem rotular como demo)
    if (de === 'gabinete' && body.autoAck !== false) {
      thread.mensagens.push({
        id: `MSG-${Date.now()}-ack`,
        de: 'secretaria',
        autor: contato.nome,
        texto: ACK_SECRETARIA[secretaria] || 'Recebido. Já estou olhando no painel e te retorno.',
        criadoEm: new Date(Date.now() + 800).toISOString(),
        ack: true,
      });
    }
