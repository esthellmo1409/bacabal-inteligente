...
async function handleModulos(ctx) {
  ...
  if (cadMatch) {
    ...
    const user = requireStaff(ctx, res, ['admin', 'platform']);
    if (!user && currentUser(req)?.papel !== 'platform') {
      // requireStaff already sent 401 if null — but platform from other city?
      if (!currentUser(req) || !['admin', 'platform'].includes(currentUser(req).papel)) return true;
    }
