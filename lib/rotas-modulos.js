...
async function handleModulos(ctx) {
  ...
  function seedThreadMensagens(slug, contato) {
    ...
    const scripts = {
      saude: [
        { de: 'secretaria', autor: c, texto: 'Bom dia. Estou no painel da Saúde.', criadoEm: horasAtras(5.2) },
        { de: 'gabinete', autor: gab, texto: 'Bom dia. Como está o atendimento hoje no hospital? Quem é o médico de plantão?', criadoEm: horasAtras(5.1) },
        { de: 'secretaria', autor: c, texto: 'No Hospital o plantão agora é o Dr. Allisson Carvalho.', criadoEm: horasAtras(5.0) },
        { de: 'secretaria', autor: c, texto: 'Atendimento com fila controlada na manhã — dentro dos conformes por enquanto. UBS São José ainda sem confirmação de plantão.', criadoEm: horasAtras(4.95) },
        { de: 'gabinete', autor: gab, texto: 'E a fila do Hospital agora?', criadoEm: horasAtras(4.7) },
        { de: 'secretaria', autor: c, texto: 'Fila em torno de 28 e espera alta. Já pedi reforço na escala da tarde.', criadoEm: horasAtras(4.6) },
        { de: 'gabinete', autor: gab, texto: 'Me avisa quando a UBS São José confirmar.', criadoEm: horasAtras(4.4) },
        { de: 'secretaria', autor: c, texto: 'Combinado. Qualquer alteração eu mando por aqui na hora.', criadoEm: horasAtras(4.3) },
      ],
      obras: [
  ...
  function respostaFluxoChat(secretaria, texto, gabineteNome, ctx = {}) {
    ...
    // —— Outras secretarias: respostas concretas no fluxo ——
    if (secretaria === 'obras') {
      if (/buraco|obra|asfalto|rua|prazo|como esta|como est[aá]|hoje/.test(t) || temSaudacao) {
        if (temSaudacao && !/buraco|obra|rua|prazo/.test(t)) {
          out.push('Obras no painel. Temos 3 buracos prioritários em andamento no Centro e São José.');
          out.push('A equipe já saiu com material — previsão de concluir o da Rua Principal ainda hoje.');
          out.push('Quer prazo de alguma rua específica?');
          return out;
        }
        out.push('Equipe de campo já está no buraco prioritário do Centro.');
        out.push('Prazo: concluir até o fim da tarde. Te mando foto do depois assim que finalizar.');
        return out;
      }
    }
