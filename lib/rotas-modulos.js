...
/** Staff + mesma cidade do tenant (platform pode cruzar). */
function requireStaffTenant(ctx, res, slug, papeis) {
  const u = requireStaff(ctx, res, papeis);
  if (!u) return null;
  if (u.papel !== 'platform' && u.cidade && slug && String(u.cidade) !== String(slug)) {
    ctx.sendJSON(res, 403, { error: 'Sessão de outra cidade — faça login de novo' });
    return null;
  }
  return u;
}
...
async function handleModulos(ctx) {
  ...
  // Auth: painéis operacionais e gabinete exigem sessão (evita pitch aberto na internet)
  if (
    /^\/api\/(obras-ops|iluminacao|saude)\//.test(pathname)
    || pathname.startsWith('/api/gabinete/')
  ) {
    const slugGate = requireTenant(req, url, res);
    if (!slugGate) return true;
    if (pathname !== '/api/gabinete/decidir') {
      let papeis = ['admin', 'prefeito', 'platform', 'secretaria', 'campo'];
      if (pathname.startsWith('/api/gabinete/')) {
        papeis = (pathname.includes('visita-surpresa'))
          ? ['admin', 'prefeito', 'platform']
          : ['admin', 'prefeito', 'platform', 'secretaria'];
      }
      if (!requireStaffTenant(ctx, res, slugGate, papeis)) return true;
    }
  }
  ...
  if (pathname === '/api/demo/preparar' && req.method === 'POST') {
    ...
    if (!slug) return true;
    if (!requireStaffTenant(ctx, res, slug, ['admin', 'prefeito', 'platform', 'secretaria'])) return true;
    // Também reforça o portfólio com antes/depois + materiais
    try {
      if (typeof ctx.ensureDemoPortfolio === 'function') ctx.ensureDemoPortfolio(slug);
      else if (typeof global.ensureDemoPortfolio === 'function') global.ensureDemoPortfolio(slug);
    } catch (_) { /* ok */ }
  ...
  if (pathname === '/api/gabinete/fila' && req.method === 'GET') {
    ...
    const agenda = eixo === 'eventos' ? ensureEventosAgenda(slug) : [];
  ...
  function ensureEventosAgenda(slug) {
    let list = readTenant(slug, 'eventos-agenda.json', []);
    const meta = readTenant(slug, 'eventos-ops-meta.json', { seedVersion: 0 });
    const CHECK_KEYS = ['palco', 'som', 'iluminacao', 'seguranca', 'limpezaPre', 'limpezaPos', 'banheiros', 'divulgacao', 'ambulancia', 'transito'];
    const defaults = [
      {
        id: 'EVT-01', nome: 'Feira Livre Municipal', local: 'Centro — Rua da Feira', data: '2026-08-11',
        horaInicio: '06:00', horaFim: '13:00', status: 'confirmado', publicoEstimado: 3000,
        responsavel: 'Luísa Freitas', telefone: '(99) 98200-1101', tipo: 'feira', destaqueGabinete: true,
        observacao: 'Prioridade do Gabinete — estrutura e limpeza pós-feira',
        checklist: { palco: true, som: false, iluminacao: true, seguranca: true, limpezaPre: true, limpezaPos: false, banheiros: true, divulgacao: true, ambulancia: false, transito: true },
      },
      {
        id: 'EVT-02', nome: 'Show Cultural Praça da Matriz', local: 'Praça da Matriz', data: '2026-08-13',
        horaInicio: '19:00', horaFim: '23:00', status: 'planejamento', publicoEstimado: 5000,
        responsavel: 'Carlos Moura', telefone: '(99) 98200-1102', tipo: 'show', destaqueGabinete: true,
        observacao: 'Falta fechar som e segurança com a guarda municipal',
        checklist: { palco: true, som: false, iluminacao: false, seguranca: false, limpezaPre: false, limpezaPos: false, banheiros: true, divulgacao: true, ambulancia: true, transito: false },
      },
      {
        id: 'EVT-03', nome: 'Semana da Cultura Popular', local: 'Centro Cultural', data: '2026-08-15',
        horaInicio: '09:00', horaFim: '21:00', status: 'confirmado', publicoEstimado: 1200,
        responsavel: 'Ana Paula Reis', telefone: '(99) 98200-1103', tipo: 'cultura', destaqueGabinete: false,
        observacao: 'Programação em 3 palcos internos',
        checklist: { palco: true, som: true, iluminacao: true, seguranca: true, limpezaPre: true, limpezaPos: false, banheiros: true, divulgacao: true, ambulancia: false, transito: false },
      },
      {
        id: 'EVT-04', nome: 'Abertura do Campeonato Amador', local: 'Estádio Municipal', data: '2026-08-18',
        horaInicio: '15:00', horaFim: '18:30', status: 'confirmado', publicoEstimado: 2000,
        responsavel: 'Roberto Dias', telefone: '(99) 98200-1104', tipo: 'esporte', destaqueGabinete: true,
        observacao: 'Coordenar trânsito na saída do estádio',
        checklist: { palco: false, som: true, iluminacao: true, seguranca: true, limpezaPre: true, limpezaPos: false, banheiros: true, divulgacao: true, ambulancia: true, transito: true },
      },
      {
        id: 'EVT-05', nome: 'Missa Campal — Padroeira', local: 'Praça da Matriz', data: '2026-08-22',
        horaInicio: '07:00', horaFim: '10:00', status: 'planejamento', publicoEstimado: 4000,
        responsavel: 'Luísa Freitas', telefone: '(99) 98200-1101', tipo: 'religioso', destaqueGabinete: true,
        observacao: 'Montagem de tendas e som na véspera',
        checklist: { palco: true, som: false, iluminacao: false, seguranca: false, limpezaPre: false, limpezaPos: false, banheiros: true, divulgacao: false, ambulancia: true, transito: true },
      },
      {
        id: 'EVT-06', nome: 'Feira de Artesanato', local: 'Calçadão do Comércio', data: '2026-08-10',
        horaInicio: '08:00', horaFim: '17:00', status: 'em_andamento', publicoEstimado: 1500,
        responsavel: 'Fernanda Lopes', telefone: '(99) 98200-1105', tipo: 'feira', destaqueGabinete: false,
        observacao: 'Evento de hoje — acompanhar limpeza e segurança',
        checklist: { palco: false, som: true, iluminacao: false, seguranca: true, limpezaPre: true, limpezaPos: false, banheiros: true, divulgacao: true, ambulancia: false, transito: true },
      },
    ];
    if (!Array.isArray(list) || !list.length || meta.seedVersion < 2) {
      list = defaults;
      writeTenant(slug, 'eventos-agenda.json', list);
      writeTenant(slug, 'eventos-ops-meta.json', { seedVersion: 2, atualizadoEm: new Date().toISOString() });
      return list;
    }
    let changed = false;
    list = list.map((e) => {
      const chk = { ...(e.checklist || {}) };
      CHECK_KEYS.forEach((k) => {
        if (typeof chk[k] !== 'boolean') {
          chk[k] = false;
          changed = true;
        }
      });
      if (!e.checklist || Object.keys(e.checklist).length < CHECK_KEYS.length) changed = true;
      return {
        ...e,
        checklist: chk,
        responsavel: e.responsavel || 'Equipe Eventos',
        horaInicio: e.horaInicio || '08:00',
        horaFim: e.horaFim || '18:00',
        tipo: e.tipo || 'evento',
      };
    });
    if (changed) writeTenant(slug, 'eventos-agenda.json', list);
    return list;
  }
  ...
  if (pathname === '/api/eventos' && req.method === 'GET') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    return sendJSON(res, 200, ensureEventosAgenda(slug));
  }
  ...
  if (pathname === '/api/eventos' && req.method === 'POST') {
    ...
    if (!user) return true;
    if (user.papel === 'secretaria' && user.secretaria && user.secretaria !== 'eventos') {
      return sendJSON(res, 403, { error: 'Apenas Cultura e Eventos pode editar a agenda' });
    }
    const body = await readBody(req);
    const list = ensureEventosAgenda(slug);
    const idx = list.findIndex((e) => e.id === body.id);
    if (idx < 0) return sendJSON(res, 404, { error: 'Evento não encontrado' });
    const ev = list[idx];
    if (body.status) ev.status = String(body.status).slice(0, 40);
    if (body.observacao !== undefined) ev.observacao = String(body.observacao || '').slice(0, 400);
    if (body.publicoEstimado !== undefined) ev.publicoEstimado = Math.max(0, Number(body.publicoEstimado) || 0);
    if (body.responsavel !== undefined) ev.responsavel = String(body.responsavel || '').slice(0, 80);
    if (body.telefone !== undefined) ev.telefone = String(body.telefone || '').slice(0, 40);
    if (body.horaInicio) ev.horaInicio = String(body.horaInicio).slice(0, 8);
    if (body.horaFim) ev.horaFim = String(body.horaFim).slice(0, 8);
    if (body.checklist && typeof body.checklist === 'object') {
      ev.checklist = { ...(ev.checklist || {}), ...body.checklist };
    }
    if (typeof body.item === 'string' && body.item) {
      ev.checklist = ev.checklist || {};
      ev.checklist[body.item] = !!body.ok;
    }
    list[idx] = ev;
    writeTenant(slug, 'eventos-agenda.json', list);
    pushLog(ctx, slug, 'eventos.update', `${ev.id} ${ev.status}`);
    return sendJSON(res, 200, { ok: true, evento: ev, agenda: list });
  }
  ...
  function respostaFluxoChat(secretaria, texto, gabineteNome, ctx = {}) {
    ...
    // —— Obras: fluxo do buraco (ex.: Djalma) + painel ao vivo ——
    if (secretaria === 'obras') {
      const painel = ctx.obras || null;
      const tot = painel?.totais || {};
      const quente = (painel?.pontosQuentes || [])[0]
        || (painel?.rota || []).find((r) => r.status !== 'concluido');
      ...
      if (falaDjalma || (/buraco/.test(t) && cobraAtraso) || (cobraAtraso && /buraco|rua|obra/.test(t))) {
        ...
        const frotaObs = (painel?.frota || []).find((f) => /brita|caçamba|cacamba/i.test(String(f.tipo || '')) && f.status !== 'disponivel');
        out.push(frotaObs
          ? `Senhor, sobre o buraco da Djalma: a equipe não chegou de manhã porque o ${frotaObs.tipo} (${frotaObs.placa}) estava ${frotaObs.status === 'oficina' ? 'em oficina' : 'atrasado na base'}.`
          : 'Senhor, sobre o buraco da Djalma: a equipe não chegou de manhã porque o caminhão de brita atrasou na base.');
      ...
      if (/buraco|obra|asfalto|rua|prazo|equipe|frota|material|brita|como esta|como est[aá]|hoje/.test(t) || temSaudacao) {
        if (temSaudacao && !/buraco|obra|rua|prazo|djalma|equipe|frota/.test(t)) {
          if (painel) {
            out.push(`${saudacaoVolta} — Obras no painel.`);
            out.push(`Hoje: ${tot.osAbertas || 0} OS aberta(s), ${tot.rotaPendentes || 0} na rota, frota ${tot.frotaLivre || 0}/${tot.frotaTotal || 0} livre.`);
            if (quente) out.push(`Ponto quente: ${quente.titulo || quente.rua || 'Djalma'} (${quente.bairro || '—'}).`);
            else out.push('Quer que eu te passe o status da Djalma ou de outra rua?');
          } else {
        ...
        if (/buraco/.test(t)) {
          out.push(quente
            ? `O buraco prioritário em aberto é ${quente.titulo || quente.rua || 'o da Djalma'}.`
            : 'O buraco prioritário em aberto é o da Djalma.');
          out.push('Equipe saindo agora com brita. Quer que eu priorize outro ponto também?');
          return out;
        }
        if (painel && (/equipe|frota|material|brita|estoque/.test(t))) {
          out.push(`Equipes ativas: ${tot.equipesEmCampo || 0}. Frota livre ${tot.frotaLivre || 0}/${tot.frotaTotal || 0}. Materiais baixos: ${tot.materiaisBaixos || 0}.`);
          return out;
        }
    ...
    if (secretaria === 'iluminacao') {
      const painel = ctx.iluminacao || null;
      const tot = painel?.totais || {};
      if (/poste|luz|apagad|como esta|hoje|praca|avenida|bairro|estoque|rota|cesta/.test(t) || temSaudacao) {
        if (temSaudacao) out.push(`${saudacaoVolta} — Iluminação no painel.`);
        if (painel) {
          out.push(`Hoje: ${tot.apagados || 0} apagado(s), ${tot.emReparo || 0} em reparo, ${tot.semEnergia || 0} sem energia.`);
          out.push(`Frota: ${tot.frotaLivre || 0}/${tot.frotaTotal || 0} livre(s). Rota com ${tot.rotaPendentes || 0} ponto(s).`);
          const crit = (painel.postes || []).find((p) => p.status === 'apagado' && p.prioridade === 'alta');
          if (crit) out.push(`Prioridade agora: poste ${crit.codigo} no ${crit.bairro} (${crit.endereco}).`);
        } else {
          out.push('Iluminação no painel. Posso te passar postes apagados, rota ou estoque de LED.');
        }
        return out.length ? out : ['Iluminação online. Me diga o poste ou o bairro.'];
      }
    }
  ...
  if (pathname === '/api/gabinete/chat' && req.method === 'POST') {
    ...
    if (de === 'gabinete' && body.autoAck !== false) {
      let saudeCtx = null;
      let iluminacaoCtx = null;
      let obrasCtx = null;
      if (secretaria === 'saude') {
        try { saudeCtx = painelSaudeHoje(slug); } catch (_) { saudeCtx = null; }
      }
      if (secretaria === 'iluminacao') {
        try { iluminacaoCtx = painelIluminacao(slug); } catch (_) { iluminacaoCtx = null; }
      }
      if (secretaria === 'obras') {
        try { obrasCtx = painelObrasOps(slug); } catch (_) { obrasCtx = null; }
      }
      const respostas = respostaFluxoChat(secretaria, texto, gabineteNome, {
        saude: saudeCtx,
        iluminacao: iluminacaoCtx,
        obras: obrasCtx,
      });
  ...
  function painelSaudeHoje(slug) {
    ...
    return {
      ...
      reclamacoes: reclamacoesAbertas.slice(0, 12).map(c => ({
        ...
        cidadao: c.cidadao || null,
        cobrancas: c.cobrancas || [],
        alertaGabineteAtivo: !!c.alertaGabineteAtivo,
        cienteGabineteEm: c.cienteGabineteEm || null,
      })),
    };
  }
  ...
  if (pathname === '/api/saude/farmacia' && req.method === 'POST') {
    ...
    if (body.acao === 'criar' || (!body.id && body.nome)) {
      const nome = String(body.nome || '').trim();
      if (!nome) return sendJSON(res, 400, { error: 'Informe o nome do medicamento/insumo' });
      const unidadeId = body.unidadeId || (unidades[0] && unidades[0].id) || null;
      const un = unidades.find((u) => u.id === unidadeId);
      const estoque = ['ok', 'baixo', 'critico'].includes(body.estoque) ? body.estoque : 'ok';
      const item = {
        id: `MED-${Date.now().toString(36).toUpperCase()}`,
        nome: nome.slice(0, 80),
        unidadeId,
        unidadeNome: un?.nome || body.unidadeNome || 'Rede',
        estoque,
        observacao: String(body.observacao || '').trim().slice(0, 200),
      };
      list.push(item);
      writeTenant(slug, 'saude-farmacia.json', list);
      pushLog(ctx, slug, 'saude.farmacia.criar', item.nome);
      return sendJSON(res, 200, painelSaudeHoje(slug));
    }
    ...
    if (body.unidadeId) {
      list[idx].unidadeId = body.unidadeId;
      const un = unidades.find((u) => u.id === body.unidadeId);
      if (un) list[idx].unidadeNome = un.nome;
    }
    writeTenant(slug, 'saude-farmacia.json', list);
  ...
  // —— Limpeza Urbana: rotas, equipes, frota, materiais ——
  function ensureLimpezaOps(slug) {
    const cfg = readTenant(slug, 'config.json', {});
    const cidade = cfg.cidade || slug;
    const hoje = new Date().toISOString().slice(0, 10);
    let data = readTenant(slug, 'limpeza-ops-hoje.json', null);
    if (data && data.data !== hoje) {
      data.data = hoje;
      data.atualizadoEm = new Date().toISOString();
      writeTenant(slug, 'limpeza-ops-hoje.json', data);
      return data;
    }
    if (data && data.seedVersion === 1) return data;
    const bairroAlt = slug === 'bomlugar' ? 'Bairro Novo' : 'São José';
    data = {
      data: hoje,
      seedVersion: 1,
      cidade,
      atualizadoEm: new Date().toISOString(),
      pontos: [
        { id: 'LP-01', titulo: 'Acúmulo de lixo — Rua Djalma', bairro: 'Centro', tipo: 'acumulo', status: 'pendente', prioridade: 'alta', rua: 'Rua Djalma' },
        { id: 'LP-02', titulo: 'Entulho na calçada', bairro: bairroAlt, tipo: 'entulho', status: 'em_execucao', prioridade: 'media', rua: 'Rua das Flores' },
        { id: 'LP-03', titulo: 'Varrição atrasada — Praça', bairro: 'Centro', tipo: 'varricao', status: 'pendente', prioridade: 'media', rua: 'Praça da Matriz' },
        { id: 'LP-04', titulo: 'Ponto de coleta irregular', bairro: 'Alto Alegre', tipo: 'coleta', status: 'pendente', prioridade: 'alta', rua: 'Av. Contorno' },
        { id: 'LP-05', titulo: 'Sacos rasgados no calçadão', bairro: 'Centro', tipo: 'acumulo', status: 'concluido', prioridade: 'baixa', rua: 'Calçadão' },
      ],
      rotas: [
        { id: 'LR-01', ordem: 1, bairro: 'Centro', tipo: 'coleta', status: 'em_andamento', equipeId: 'EQ-LIM-01', turno: 'manhã', km: 12 },
        { id: 'LR-02', ordem: 2, bairro: bairroAlt, tipo: 'varricao', status: 'pendente', equipeId: 'EQ-LIM-02', turno: 'manhã', km: 8 },
        { id: 'LR-03', ordem: 3, bairro: 'Alto Alegre', tipo: 'coleta', status: 'pendente', equipeId: 'EQ-LIM-01', turno: 'tarde', km: 15 },
        { id: 'LR-04', ordem: 4, bairro: 'Centro', tipo: 'entulho', status: 'pendente', equipeId: 'EQ-LIM-03', turno: 'tarde', km: 6 },
      ],
      equipes: [
        { id: 'EQ-LIM-01', nome: 'Equipe Coleta Norte', lider: 'José Amaral', status: 'em_campo', membros: 5, radio: 'Canal 2', veiculoId: 'LIM-VEI-01' },
        { id: 'EQ-LIM-02', nome: 'Equipe Varrição Centro', lider: 'Fernanda Dias', status: 'em_deslocamento', membros: 4, radio: 'Canal 2', veiculoId: 'LIM-VEI-02' },
        { id: 'EQ-LIM-03', nome: 'Equipe Entulho', lider: 'Paulo Cruz', status: 'disponivel', membros: 3, radio: 'Canal 4', veiculoId: 'LIM-VEI-03' },
      ],
      frota: [
        { id: 'LIM-VEI-01', placa: 'NMA-7G11', tipo: 'Compactador', status: 'em_rota', motorista: 'José Amaral', equipe: 'EQ-LIM-01' },
        { id: 'LIM-VEI-02', placa: 'NMA-8H22', tipo: 'Varredeira', status: 'em_rota', motorista: 'Fernanda Dias', equipe: 'EQ-LIM-02' },
        { id: 'LIM-VEI-03', placa: 'NMA-9I33', tipo: 'Caçamba', status: 'disponivel', motorista: 'Paulo Cruz', equipe: 'EQ-LIM-03' },
        { id: 'LIM-VEI-04', placa: 'NMA-0J44', tipo: 'Compactador', status: 'oficina', motorista: '—', equipe: '—' },
      ],
      materiais: [
        { id: 'LIM-MAT-01', nome: 'Saco de lixo 100L', qtd: 420, minimo: 200, unidade: 'un' },
        { id: 'LIM-MAT-02', nome: 'Saco de lixo 200L', qtd: 85, minimo: 100, unidade: 'un' },
        { id: 'LIM-MAT-03', nome: 'Vassoura de piaçava', qtd: 48, minimo: 20, unidade: 'un' },
        { id: 'LIM-MAT-04', nome: 'Luvas EPI', qtd: 60, minimo: 40, unidade: 'par' },
        { id: 'LIM-MAT-05', nome: 'Combustível (reserva)', qtd: 180, minimo: 250, unidade: 'L' },
      ],
      checklist: [
        { id: 'LC-01', label: 'Briefing das equipes', ok: true },
        { id: 'LC-02', label: 'Caminhões abastecidos', ok: true },
        { id: 'LC-03', label: 'Rota de coleta publicada', ok: true },
        { id: 'LC-04', label: 'EPIs conferidos', ok: false },
        { id: 'LC-05', label: 'Ponto crítico Centro vistoriado', ok: false },
        { id: 'LC-06', label: 'Limpeza pós-feira alinhada', ok: false },
      ],
    };
    writeTenant(slug, 'limpeza-ops-hoje.json', data);
    return data;
  ...
  function painelLimpezaOps(slug) {
    const data = ensureLimpezaOps(slug);
    const todosLimp = readTenant(slug, 'chamados.json', []).map(normalizeChamado)
      .filter((c) => c.secretaria === 'limpeza');
    const abertos = todosLimp.filter((c) => c.status !== 'concluido' && c.status !== 'cancelado');
    const concluidos = todosLimp
      .filter((c) => c.status === 'concluido')
      .sort((a, b) => new Date(b.atualizadoEm || b.criadoEm || 0) - new Date(a.atualizadoEm || a.criadoEm || 0));
    const totais = {
      pontosAbertos: (data.pontos || []).filter((p) => p.status !== 'concluido').length,
      pontosAlta: (data.pontos || []).filter((p) => p.status !== 'concluido' && p.prioridade === 'alta').length,
      rotasPendentes: (data.rotas || []).filter((r) => r.status !== 'concluido').length,
      rotasAndamento: (data.rotas || []).filter((r) => r.status === 'em_andamento').length,
      equipesEmCampo: (data.equipes || []).filter((e) => e.status === 'em_campo' || e.status === 'em_deslocamento').length,
      equipesTotal: (data.equipes || []).length,
      ...
      checklistPendentes: (data.checklist || []).filter((c) => !c.ok).length,
      chamadosAbertos: abertos.length,
      chamadosConcluidos: concluidos.length,
    };
    const alertas = [];
    if (totais.pontosAlta) alertas.push(`${totais.pontosAlta} ponto(s) de limpeza prioridade alta`);
    if (totais.materiaisBaixos) alertas.push(`${totais.materiaisBaixos} material(is) abaixo do mínimo`);
    if ((data.frota || []).some((f) => f.status === 'oficina')) alertas.push('Veículo em oficina');
    if (totais.checklistPendentes) alertas.push(`${totais.checklistPendentes} item(ns) do checklist pendente(s)`);
    if (totais.chamadosAbertos >= 5) alertas.push(`${totais.chamadosAbertos} chamados abertos na fila`);
    const mapCh = (c) => ({
      id: c.id,
      protocolo: c.protocolo,
      titulo: c.titulo,
    ...
    return {
      ...
      totais,
      alertas,
      chamados: [...abertos, ...concluidos.slice(0, 10)].slice(0, 24).map(mapCh),
    };
  }

  if (pathname === '/api/limpeza/hoje' && req.method === 'GET') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    return sendJSON(res, 200, painelLimpezaOps(slug));
  }

  if (pathname === '/api/limpeza/ponto' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const user = requireStaff(ctx, res, ['admin', 'secretaria']);
    if (!user) return true;
    const body = await readBody(req);
    const data = ensureLimpezaOps(slug);
    const idx = data.pontos.findIndex((p) => p.id === body.id);
    if (idx < 0) return sendJSON(res, 404, { error: 'Ponto não encontrado' });
    if (body.status) data.pontos[idx].status = body.status;
    if (body.prioridade) data.pontos[idx].prioridade = body.prioridade;
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'limpeza-ops-hoje.json', data);
    pushLog(ctx, slug, 'limpeza.ponto', `${body.id} ${data.pontos[idx].status}`);
    return sendJSON(res, 200, painelLimpezaOps(slug));
  }

  if (pathname === '/api/limpeza/rota' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const user = requireStaff(ctx, res, ['admin', 'secretaria']);
    if (!user) return true;
    const body = await readBody(req);
    const data = ensureLimpezaOps(slug);
    const idx = data.rotas.findIndex((r) => r.id === body.id);
    if (idx < 0) return sendJSON(res, 404, { error: 'Rota não encontrada' });
    if (body.status) data.rotas[idx].status = body.status;
    if (body.equipeId !== undefined) data.rotas[idx].equipeId = body.equipeId || null;
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'limpeza-ops-hoje.json', data);
    pushLog(ctx, slug, 'limpeza.rota', `${body.id} ${data.rotas[idx].status}`);
    return sendJSON(res, 200, painelLimpezaOps(slug));
  }

  if (pathname === '/api/limpeza/equipe' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const user = requireStaff(ctx, res, ['admin', 'secretaria']);
    if (!user) return true;
    const body = await readBody(req);
    const data = ensureLimpezaOps(slug);
    const idx = data.equipes.findIndex((e) => e.id === body.id);
    if (idx < 0) return sendJSON(res, 404, { error: 'Equipe não encontrada' });
    if (body.status) data.equipes[idx].status = body.status;
    if (body.membros !== undefined) data.equipes[idx].membros = Math.max(1, Number(body.membros) || 1);
    if (body.radio !== undefined) data.equipes[idx].radio = String(body.radio || '').slice(0, 40);
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'limpeza-ops-hoje.json', data);
    pushLog(ctx, slug, 'limpeza.equipe', `${body.id} ${data.equipes[idx].status}`);
    return sendJSON(res, 200, painelLimpezaOps(slug));
  }

  if (pathname === '/api/limpeza/frota' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const user = requireStaff(ctx, res, ['admin', 'secretaria']);
    if (!user) return true;
    const body = await readBody(req);
    const data = ensureLimpezaOps(slug);
    const idx = data.frota.findIndex((f) => f.id === body.id);
    if (idx < 0) return sendJSON(res, 404, { error: 'Veículo não encontrado' });
    if (body.status) data.frota[idx].status = body.status;
    if (body.motorista !== undefined) data.frota[idx].motorista = String(body.motorista || '').slice(0, 80);
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'limpeza-ops-hoje.json', data);
    pushLog(ctx, slug, 'limpeza.frota', `${data.frota[idx].placa} ${data.frota[idx].status}`);
    return sendJSON(res, 200, painelLimpezaOps(slug));
  }

  if (pathname === '/api/limpeza/material' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const user = requireStaff(ctx, res, ['admin', 'secretaria']);
    if (!user) return true;
    const body = await readBody(req);
    const data = ensureLimpezaOps(slug);
    const idx = data.materiais.findIndex((m) => m.id === body.id);
    if (idx < 0) return sendJSON(res, 404, { error: 'Material não encontrado' });
    if (body.qtd !== undefined) data.materiais[idx].qtd = Math.max(0, Number(body.qtd) || 0);
    if (body.minimo !== undefined) data.materiais[idx].minimo = Math.max(0, Number(body.minimo) || 0);
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'limpeza-ops-hoje.json', data);
    pushLog(ctx, slug, 'limpeza.material', `${data.materiais[idx].nome}=${data.materiais[idx].qtd}`);
    return sendJSON(res, 200, painelLimpezaOps(slug));
  }

  if (pathname === '/api/limpeza/checklist' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const user = requireStaff(ctx, res, ['admin', 'secretaria']);
    if (!user) return true;
    const body = await readBody(req);
    const data = ensureLimpezaOps(slug);
    const idx = data.checklist.findIndex((c) => c.id === body.id);
    if (idx < 0) return sendJSON(res, 404, { error: 'Item não encontrado' });
    data.checklist[idx].ok = !!body.ok;
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'limpeza-ops-hoje.json', data);
    pushLog(ctx, slug, 'limpeza.checklist', `${data.checklist[idx].label}=${data.checklist[idx].ok}`);
    return sendJSON(res, 200, painelLimpezaOps(slug));
  }

  // —— Iluminação Pública: postes, frota, estoque, rota ——
  function ensureIluminacao(slug) {
    const cfg = readTenant(slug, 'config.json', {});
    const cidade = cfg.cidade || slug;
    let data = readTenant(slug, 'iluminacao-hoje.json', null);
    const hoje = new Date().toISOString().slice(0, 10);
    if (data && data.data !== hoje) {
      data.data = hoje;
      data.atualizadoEm = new Date().toISOString();
      writeTenant(slug, 'iluminacao-hoje.json', data);
      return data;
    }
    if (data && data.seedVersion === 1) return data;
    if (!data || data.seedVersion !== 1) {
      const postes = [
        { id: 'P-1201', codigo: '1201', bairro: 'Centro', endereco: 'Av. Principal, esquina com 7 de Setembro', status: 'apagado', prioridade: 'alta', lat: cfg.lat || -4.28, lng: cfg.lng || -44.8 },
        { id: 'P-1208', codigo: '1208', bairro: 'Centro', endereco: 'Praça central — poste 08', status: 'apagado', prioridade: 'alta', lat: (cfg.lat || -4.28) + 0.002, lng: (cfg.lng || -44.8) + 0.001 },
        { id: 'P-1457', codigo: '1457', bairro: slug === 'bomlugar' ? 'Bairro Novo' : 'São José', endereco: 'Rua das Flores, 210', status: 'em_reparo', prioridade: 'media', lat: (cfg.lat || -4.28) - 0.01, lng: (cfg.lng || -44.8) + 0.01 },
        { id: 'P-1510', codigo: '1510', bairro: slug === 'bomlugar' ? 'Bairro Novo' : 'São José', endereco: 'Travessa Norte, 45', status: 'apagado', prioridade: 'media', lat: (cfg.lat || -4.28) - 0.012, lng: (cfg.lng || -44.8) + 0.008 },
        { id: 'P-1602', codigo: '1602', bairro: 'Centro', endereco: 'Rua do Comércio, 88', status: 'ok', prioridade: 'baixa', lat: (cfg.lat || -4.28) + 0.001, lng: (cfg.lng || -44.8) - 0.002 },
        { id: 'P-1711', codigo: '1711', bairro: slug === 'bomlugar' ? 'Centro' : 'Alto Alegre', endereco: 'Av. Contorno, km 2', status: 'sem_energia', prioridade: 'alta', lat: (cfg.lat || -4.28) + 0.015, lng: (cfg.lng || -44.8) - 0.005 },
        { id: 'P-1820', codigo: '1820', bairro: 'Centro', endereco: 'Rua da Igreja', status: 'ok', prioridade: 'baixa', lat: (cfg.lat || -4.28) - 0.003, lng: (cfg.lng || -44.8) + 0.003 },
        { id: 'P-1905', codigo: '1905', bairro: slug === 'bomlugar' ? 'Bairro Novo' : 'São José', endereco: 'Rua Nova, 12', status: 'apagado', prioridade: 'baixa', lat: (cfg.lat || -4.28) - 0.014, lng: (cfg.lng || -44.8) + 0.012 },
      ].map((p) => ({ ...p, atualizadoEm: new Date().toISOString() }));
      data = {
        data: hoje,
        seedVersion: 1,
        cidade,
        ...
        postes,
        frota: [
          { id: 'ILU-VEI-01', placa: 'NMA-4D89', tipo: 'Cesta aérea', status: 'disponivel', motorista: 'Paulo Souza', equipe: 'EQ-ILU-01' },
          { id: 'ILU-VEI-02', placa: 'NMA-5E12', tipo: 'Cesta aérea', status: 'em_rota', motorista: 'Ricardo Alves', equipe: 'EQ-ILU-01' },
          { id: 'ILU-VEI-03', placa: 'NMA-6F34', tipo: 'Utilitário', status: 'oficina', motorista: '—', equipe: '—' },
        ],
        estoque: [
          { id: 'ILU-MAT-01', nome: 'Lâmpada LED 100W', qtd: 85, minimo: 40, unidade: 'un' },
          { id: 'ILU-MAT-02', nome: 'Lâmpada LED 150W', qtd: 22, minimo: 25, unidade: 'un' },
          { id: 'ILU-MAT-03', nome: 'Reator / driver LED', qtd: 18, minimo: 10, unidade: 'un' },
          { id: 'ILU-MAT-04', nome: 'Cabo PP 2,5mm', qtd: 120, minimo: 50, unidade: 'm' },
          { id: 'ILU-MAT-05', nome: 'Braço de poste', qtd: 6, minimo: 8, unidade: 'un' },
        ],
        rota: postes
          .filter((p) => p.status === 'apagado' || p.status === 'em_reparo' || p.status === 'sem_energia')
          .slice(0, 5)
          .map((p, i) => ({
            ordem: i + 1,
            posteId: p.id,
            ...
            bairro: p.bairro,
            endereco: p.endereco,
            status: i === 0 ? 'em_atendimento' : 'pendente',
  ...
  function painelIluminacao(slug) {
    const data = ensureIluminacao(slug);
    const todosIlum = readTenant(slug, 'chamados.json', [])
      .map(normalizeChamado)
      .filter((c) => c.secretaria === 'iluminacao');
    const chamadosAbertos = todosIlum.filter((c) => isPendente(c.status));
    const chamadosConcluidos = todosIlum
      .filter((c) => !isPendente(c.status) && (c.fotoDepois || (c.demoFixo && String(c.demoFixo).startsWith('cenario-'))))
      .sort((a, b) => new Date(b.atualizadoEm || b.criadoEm || 0) - new Date(a.atualizadoEm || a.criadoEm || 0))
      .slice(0, 6);
    const chamados = [...chamadosAbertos, ...chamadosConcluidos];
    ...
    const totais = {
      ...
      ok: postes.filter((p) => p.status === 'ok').length,
    ...
    const alertas = [];
    postes.filter((p) => p.status === 'apagado' && p.prioridade === 'alta').forEach((p) => {
      alertas.push({ tipo: 'poste', nivel: 'alto', mensagem: `Poste ${p.codigo} apagado — ${p.bairro}` });
    });
    postes.filter((p) => p.status === 'sem_energia').forEach((p) => {
      alertas.push({ tipo: 'rede', nivel: 'alto', mensagem: `Poste ${p.codigo} sem energia — possível rede` });
    });
    (data.estoque || []).filter((e) => (e.qtd || 0) < (e.minimo || 0)).forEach((e) => {
      alertas.push({ tipo: 'estoque', nivel: 'medio', mensagem: `${e.nome}: estoque baixo (${e.qtd}/${e.minimo})` });
    });
    (data.frota || []).filter((f) => f.status === 'oficina').forEach((f) => {
      alertas.push({ tipo: 'frota', nivel: 'medio', mensagem: `${f.placa} em oficina` });
    });
    return {
      data: data.data,
      atualizadoEm: data.atualizadoEm,
      ...
      rota: data.rota,
      alertas,
      chamados: chamados.slice(0, 16).map((c) => ({
        id: c.id,
        protocolo: c.protocolo,
        titulo: c.titulo,
        categoria: c.categoria,
  ...
  if (pathname === '/api/iluminacao/hoje' && req.method === 'GET') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    return sendJSON(res, 200, painelIluminacao(slug));
  }

  if (pathname === '/api/iluminacao/poste' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const body = await readBody(req);
    const data = ensureIluminacao(slug);
    const idx = data.postes.findIndex((p) => p.id === body.id || p.codigo === String(body.codigo || ''));
    if (idx < 0) return sendJSON(res, 404, { error: 'Poste não encontrado' });
    if (body.status) data.postes[idx].status = body.status;
    if (body.prioridade) data.postes[idx].prioridade = body.prioridade;
    if (body.bairro) data.postes[idx].bairro = body.bairro;
    if (body.endereco) data.postes[idx].endereco = body.endereco;
    data.postes[idx].atualizadoEm = new Date().toISOString();
    data.atualizadoEm = data.postes[idx].atualizadoEm;
    // sync rota item
    const rIdx = (data.rota || []).findIndex((r) => r.posteId === data.postes[idx].id);
    if (rIdx >= 0 && body.status === 'ok') data.rota[rIdx].status = 'concluido';
    if (rIdx >= 0 && body.status === 'em_reparo') data.rota[rIdx].status = 'em_atendimento';
    writeTenant(slug, 'iluminacao-hoje.json', data);
    pushLog(ctx, slug, 'iluminacao.poste', `${data.postes[idx].codigo} ${data.postes[idx].status}`);
    return sendJSON(res, 200, painelIluminacao(slug));
  }

  if (pathname === '/api/iluminacao/frota' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const body = await readBody(req);
    const data = ensureIluminacao(slug);
    ...
    if (idx < 0) return sendJSON(res, 404, { error: 'Veículo não encontrado' });
    if (body.status) data.frota[idx].status = body.status;
    if (body.motorista != null) data.frota[idx].motorista = String(body.motorista).trim();
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'iluminacao-hoje.json', data);
    pushLog(ctx, slug, 'iluminacao.frota', `${data.frota[idx].placa} ${data.frota[idx].status}`);
    return sendJSON(res, 200, painelIluminacao(slug));
  }

  if (pathname === '/api/iluminacao/estoque' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const body = await readBody(req);
    const data = ensureIluminacao(slug);
    ...
    if (body.acao === 'criar' || (!body.id && body.nome)) {
      const nome = String(body.nome || '').trim();
      if (!nome) return sendJSON(res, 400, { error: 'Informe o nome do item' });
      const item = {
        id: `ILU-MAT-${Date.now().toString(36).toUpperCase()}`,
        nome: nome.slice(0, 80),
        qtd: Math.max(0, Number(body.qtd) || 0),
        minimo: Math.max(0, Number(body.minimo) || 0),
        unidade: String(body.unidade || 'un').trim().slice(0, 12) || 'un',
      };
      data.estoque.push(item);
      data.atualizadoEm = new Date().toISOString();
      writeTenant(slug, 'iluminacao-hoje.json', data);
      pushLog(ctx, slug, 'iluminacao.estoque.criar', item.nome);
      return sendJSON(res, 200, painelIluminacao(slug));
    }

    const idx = data.estoque.findIndex((e) => e.id === body.id);
    if (idx < 0) return sendJSON(res, 404, { error: 'Item não encontrado' });
    if (body.qtd != null) data.estoque[idx].qtd = Math.max(0, Number(body.qtd) || 0);
    if (body.minimo != null) data.estoque[idx].minimo = Math.max(0, Number(body.minimo) || 0);
    if (body.nome) data.estoque[idx].nome = String(body.nome).trim().slice(0, 80);
    if (body.unidade) data.estoque[idx].unidade = String(body.unidade).trim().slice(0, 12);
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'iluminacao-hoje.json', data);
    pushLog(ctx, slug, 'iluminacao.estoque', `${data.estoque[idx].nome}=${data.estoque[idx].qtd}`);
    return sendJSON(res, 200, painelIluminacao(slug));
  }

  if (pathname === '/api/iluminacao/rota' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const body = await readBody(req);
    const data = ensureIluminacao(slug);
    if (body.acao === 'rebuild') {
      data.rota = (data.postes || [])
        .filter((p) => p.status === 'apagado' || p.status === 'em_reparo' || p.status === 'sem_energia')
      ...
      const idx = (data.rota || []).findIndex((r) => r.posteId === body.posteId || r.codigo === String(body.codigo || ''));
      if (idx < 0) return sendJSON(res, 404, { error: 'Item da rota não encontrado' });
      if (body.status) data.rota[idx].status = body.status;
      if (body.status === 'concluido') {
        const pIdx = data.postes.findIndex((p) => p.id === data.rota[idx].posteId);
        if (pIdx >= 0) {
          data.postes[pIdx].status = 'ok';
          data.postes[pIdx].atualizadoEm = new Date().toISOString();
        }
      }
      if (body.status === 'em_atendimento') {
        const pIdx = data.postes.findIndex((p) => p.id === data.rota[idx].posteId);
        if (pIdx >= 0) data.postes[pIdx].status = 'em_reparo';
      }
    }
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'iluminacao-hoje.json', data);
    pushLog(ctx, slug, 'iluminacao.rota', body.acao || body.status || 'update');
    return sendJSON(res, 200, painelIluminacao(slug));
  }

  // ——— Secretaria de Obras (painel operacional) ———
  function ensureObrasOps(slug) {
    const cfg = readTenant(slug, 'config.json', {});
    const cidade = cfg.cidade || slug;
    let data = readTenant(slug, 'obras-ops-hoje.json', null);
    const hoje = new Date().toISOString().slice(0, 10);
    if (data && data.data !== hoje) {
      data.data = hoje;
      data.atualizadoEm = new Date().toISOString();
      writeTenant(slug, 'obras-ops-hoje.json', data);
      return data;
    }
    if (data && data.seedVersion === 3) return data;
    if (!data || data.seedVersion !== 3) {
      const bairroAlt = slug === 'bomlugar' ? 'Bairro Novo' : 'São José';
      data = {
        data: hoje,
        seedVersion: 3,
        ...
        pontosQuentes: [
          {
            ...
            equipeId: 'EQ-OBR-01',
            motivoAtraso: 'Caminhão de brita atrasou na base',
            prazoHoje: '16:00',
            simBuraco: true,
            lat: cfg.lat || -4.28,
            lng: cfg.lng || -44.8,
          ...
          {
            id: 'PQ-CALCADA',
            titulo: 'Calçada danificada — Praça',
            rua: 'Praça central',
            bairro: 'Centro',
            tipo: 'calcada',
            prioridade: 'media',
            ...
            motivoAtraso: null,
            prazoHoje: '18:00',
            lat: (cfg.lat || -4.28) + 0.002,
            lng: (cfg.lng || -44.8) + 0.001,
          },
          {
            id: 'PQ-DRENO',
            titulo: 'Boca de lobo entupida',
            rua: 'Av. Contorno',
            bairro: bairroAlt,
            tipo: 'drenagem',
            prioridade: 'alta',
            ...
            motivoAtraso: null,
            prazoHoje: '17:00',
            lat: (cfg.lat || -4.28) - 0.01,
            lng: (cfg.lng || -44.8) + 0.008,
          },
        ],
        equipes: [
          {
            id: 'EQ-OBR-01', nome: 'Equipe Asfalto 01', lider: 'José Carlos', status: 'em_campo',
            veiculoId: 'OBR-VEI-01', pontoId: 'PQ-DJALMA', membros: 4, especialidade: 'Tapa-buraco / asfalto frio',
            telefone: '(99) 98111-0101', radio: 'Canal 3', observacao: 'Prioridade Djalma até 16h',
          },
          {
            id: 'EQ-OBR-02', nome: 'Equipe Calçada', lider: 'Ana Paula', status: 'base',
            veiculoId: 'OBR-VEI-03', pontoId: 'PQ-CALCADA', membros: 3, especialidade: 'Calçada / meio-fio',
            telefone: '(99) 98111-0202', radio: 'Canal 3', observacao: '',
          },
          {
            id: 'EQ-OBR-03', nome: 'Equipe Drenagem', lider: 'Roberto Lima', status: 'disponivel',
            veiculoId: null, pontoId: null, membros: 5, especialidade: 'Bueiro / drenagem',
            telefone: '(99) 98111-0303', radio: 'Canal 5', observacao: 'Disponível para reforço',
          },
          {
            id: 'EQ-OBR-04', nome: 'Equipe Sinalização', lider: 'Marcos Vieira', status: 'em_deslocamento',
            veiculoId: 'OBR-VEI-02', pontoId: null, membros: 2, especialidade: 'Cone / desvio de tráfego',
            telefone: '(99) 98111-0404', radio: 'Canal 4', observacao: 'Apoio às equipes em via',
          },
        ],
        frota: [
          { id: 'OBR-VEI-01', placa: 'NMA-1A23', tipo: 'Caminhão brita', status: 'em_rota', motorista: 'Paulo Souza', equipe: 'EQ-OBR-01' },
          { id: 'OBR-VEI-02', placa: 'NMA-2B45', tipo: 'Caçamba', status: 'disponivel', motorista: 'Ricardo Alves', equipe: '—' },
          { id: 'OBR-VEI-03', placa: 'NMA-3C67', tipo: 'Utilitário', status: 'em_rota', motorista: 'Ana Paula', equipe: 'EQ-OBR-02' },
          { id: 'OBR-VEI-04', placa: 'NMA-4D89', tipo: 'Retroescavadeira', status: 'oficina', motorista: '—', equipe: '—' },
        ],
        materiais: [
          { id: 'OBR-MAT-01', nome: 'Brita 1', qtd: 12, minimo: 8, unidade: 'm³' },
          { id: 'OBR-MAT-02', nome: 'Asfalto frio', qtd: 4, minimo: 6, unidade: 't' },
          { id: 'OBR-MAT-03', nome: 'Areia', qtd: 18, minimo: 10, unidade: 'm³' },
          { id: 'OBR-MAT-04', nome: 'Cimento', qtd: 35, minimo: 20, unidade: 'sc' },
          { id: 'OBR-MAT-05', nome: 'Concreto usinado', qtd: 2, minimo: 3, unidade: 'm³' },
        ],
        checklist: [
          { id: 'CHK-01', label: 'Briefing das equipes', ok: true },
          { id: 'CHK-02', label: 'Veículos abastecidos', ok: true },
          { id: 'CHK-03', label: 'Brita liberada na base', ok: false },
          { id: 'CHK-04', label: 'EPIs conferidos', ok: true },
          { id: 'CHK-05', label: 'Fotos antes dos pontos quentes', ok: false },
        ],
        rota: [],
      };
      data.rota = data.pontosQuentes
  ...
  function painelObrasOps(slug) {
    const data = ensureObrasOps(slug);
    const todosChamados = readTenant(slug, 'chamados.json', []).map(normalizeChamado);
    const daObras = todosChamados.filter((c) => c.secretaria === 'obras');
    // Fila aberta + concluídos recentes (portfólio com antes/depois + materiais)
    const chamadosAbertos = daObras.filter((c) => isPendente(c.status));
    const chamadosConcluidos = daObras
      .filter((c) => !isPendente(c.status) && (c.fotoDepois || (c.demoFixo && String(c.demoFixo).startsWith('cenario-'))))
      .sort((a, b) => new Date(b.atualizadoEm || b.criadoEm || 0) - new Date(a.atualizadoEm || a.criadoEm || 0))
      .slice(0, 8);
    const chamados = [...chamadosAbertos, ...chamadosConcluidos];
    const obras = readTenant(slug, 'obras.json', []);
    const demoBuraco = todosChamados.find((c) => c.demoFixo === 'buraco-foto')
      || chamadosAbertos.find((c) => /djalma|buraco/i.test(`${c.endereco || ''} ${c.titulo || ''} ${c.descricao || ''}`));

    const mapChamadoOps = (c) => ({
      id: c.id,
      protocolo: c.protocolo,
      titulo: c.titulo,
      categoria: c.categoria,
    ...
    let pontos = (data.pontosQuentes || []).map((p) => {
      if (!(p.simBuraco || p.id === 'PQ-DJALMA') || !demoBuraco) return { ...p };
      return {
        ...p,
        chamadoId: demoBuraco.id,
        protocolo: demoBuraco.protocolo,
        cidadaoChamouEm: demoBuraco.criadoEm,
        cidadao: demoBuraco.cidadao || null,
        foto: demoBuraco.fotoAntes || demoBuraco.foto || null,
        fotoAntes: demoBuraco.fotoAntes || demoBuraco.foto || null,
        fotoDepois: demoBuraco.fotoDepois || null,
        fotosAntes: demoBuraco.fotosAntes || null,
        fotosDepois: demoBuraco.fotosDepois || null,
        historico: demoBuraco.historico || [],
        atualizadoEm: demoBuraco.atualizadoEm || null,
        statusChamado: demoBuraco.status,
        descricao: demoBuraco.descricao || p.titulo,
        tituloChamado: demoBuraco.titulo,
        enderecoChamado: demoBuraco.endereco || p.rua,
        demoFixo: demoBuraco.demoFixo || 'buraco-foto',
      };
    });

    const totais = {
      osAbertas: chamadosAbertos.length,
      ...
      rotaPendentes: (data.rota || []).filter((r) => r.status !== 'concluido').length,
      checklistPendentes: (data.checklist || []).filter((c) => !c.ok).length,
      obrasAndamento: obras.filter((o) => o.status === 'em_andamento').length,
      obrasAtrasadas: obras.filter((o) => o.status === 'atrasada').length,
      obrasTotal: obras.length,
    };
    const alertas = [];
    pontos.filter((p) => p.prioridade === 'alta' && p.status !== 'concluido').forEach((p) => {
      const quando = p.cidadaoChamouEm
        ? ` · cidadão chamou ${new Date(p.cidadaoChamouEm).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}`
        : '';
      alertas.push({ tipo: 'ponto', nivel: 'alto', mensagem: `${p.titulo} — ${p.bairro} (${p.status})${quando}` });
    });
    (data.materiais || []).filter((m) => (m.qtd || 0) < (m.minimo || 0)).forEach((m) => {
      alertas.push({ tipo: 'material', nivel: 'medio', mensagem: `${m.nome}: estoque baixo (${m.qtd}/${m.minimo} ${m.unidade})` });
    });
    (data.frota || []).filter((f) => f.status === 'oficina').forEach((f) => {
      alertas.push({ tipo: 'frota', nivel: 'medio', mensagem: `${f.placa} (${f.tipo}) em oficina` });
    });
    obras.filter((o) => o.status === 'atrasada').forEach((o) => {
      alertas.push({ tipo: 'obra', nivel: 'alto', mensagem: `Obra atrasada: ${o.nome} (${o.percentual}%)` });
    });
    (data.checklist || []).filter((c) => !c.ok).forEach((c) => {
      alertas.push({ tipo: 'checklist', nivel: 'baixo', mensagem: `Checklist pendente: ${c.label}` });
    });

    let listaChamados = chamados.map(mapChamadoOps);
    if (demoBuraco && !listaChamados.find((c) => c.id === demoBuraco.id)) {
      listaChamados = [mapChamadoOps(demoBuraco), ...listaChamados];
    }
    ...
    return {
      ...
      rota: data.rota,
      alertas,
      simBuraco: demoBuraco ? mapChamadoOps(demoBuraco) : null,
      obras: obras.map((o) => ({
        id: o.id,
        nome: o.nome,
        status: o.status,
      ...
      chamados: listaChamados.slice(0, 20),
    };
  }

  if (pathname === '/api/obras-ops/hoje' && req.method === 'GET') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    return sendJSON(res, 200, painelObrasOps(slug));
  }

  if (pathname === '/api/obras-ops/ponto' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const body = await readBody(req);
    const data = ensureObrasOps(slug);
    const idx = data.pontosQuentes.findIndex((p) => p.id === body.id);
    if (idx < 0) return sendJSON(res, 404, { error: 'Ponto não encontrado' });
    if (body.status) data.pontosQuentes[idx].status = body.status;
    if (body.prioridade) data.pontosQuentes[idx].prioridade = body.prioridade;
    if (body.equipeId !== undefined) data.pontosQuentes[idx].equipeId = body.equipeId || null;
    if (body.motivoAtraso !== undefined) data.pontosQuentes[idx].motivoAtraso = body.motivoAtraso || null;
    if (body.prazoHoje) data.pontosQuentes[idx].prazoHoje = body.prazoHoje;
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'obras-ops-hoje.json', data);
    pushLog(ctx, slug, 'obras.ponto', `${data.pontosQuentes[idx].id} ${data.pontosQuentes[idx].status}`);
    return sendJSON(res, 200, painelObrasOps(slug));
  }

  if (pathname === '/api/obras-ops/equipe' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const body = await readBody(req);
    const data = ensureObrasOps(slug);
    const idx = data.equipes.findIndex((e) => e.id === body.id);
    if (idx < 0) return sendJSON(res, 404, { error: 'Equipe não encontrada' });
    const eq = data.equipes[idx];
    if (body.status) eq.status = body.status;
    if (body.pontoId !== undefined) eq.pontoId = body.pontoId || null;
    if (body.veiculoId !== undefined) {
      eq.veiculoId = body.veiculoId || null;
      (data.frota || []).forEach((f) => {
        if (f.equipe === eq.id && f.id !== eq.veiculoId) f.equipe = '—';
        if (eq.veiculoId && f.id === eq.veiculoId) {
          f.equipe = eq.id;
          if (eq.status === 'em_campo' || eq.status === 'em_deslocamento') f.status = 'em_rota';
        }
      });
    }
    if (body.membros !== undefined) eq.membros = Math.max(1, Math.min(30, Number(body.membros) || 1));
    if (body.telefone !== undefined) eq.telefone = String(body.telefone || '').slice(0, 40);
    if (body.radio !== undefined) eq.radio = String(body.radio || '').slice(0, 40);
    if (body.observacao !== undefined) eq.observacao = String(body.observacao || '').slice(0, 240);
    if (body.especialidade !== undefined) eq.especialidade = String(body.especialidade || '').slice(0, 80);
    if (body.lider !== undefined) eq.lider = String(body.lider || '').slice(0, 80);
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'obras-ops-hoje.json', data);
    pushLog(ctx, slug, 'obras.equipe', `${eq.id} ${eq.status}`);
    return sendJSON(res, 200, painelObrasOps(slug));
  }

  if (pathname === '/api/obras-ops/frota' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const body = await readBody(req);
    const data = ensureObrasOps(slug);
    ...
    if (idx < 0) return sendJSON(res, 404, { error: 'Veículo não encontrado' });
    if (body.status) data.frota[idx].status = body.status;
    if (body.motorista !== undefined) data.frota[idx].motorista = body.motorista;
    if (body.equipe !== undefined) data.frota[idx].equipe = body.equipe;
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'obras-ops-hoje.json', data);
    pushLog(ctx, slug, 'obras.frota', `${data.frota[idx].placa} ${data.frota[idx].status}`);
    return sendJSON(res, 200, painelObrasOps(slug));
  }

  if (pathname === '/api/obras-ops/material' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const body = await readBody(req);
    const data = ensureObrasOps(slug);
    if (!Array.isArray(data.materiais)) data.materiais = [];

    if (body.acao === 'criar' || (!body.id && body.nome)) {
      const nome = String(body.nome || '').trim();
      if (!nome) return sendJSON(res, 400, { error: 'Informe o nome do material' });
      const item = {
        id: `OBR-MAT-${Date.now().toString(36).toUpperCase()}`,
        nome: nome.slice(0, 80),
        qtd: Math.max(0, Number(body.qtd) || 0),
        minimo: Math.max(0, Number(body.minimo) || 0),
        unidade: String(body.unidade || 'un').trim().slice(0, 12) || 'un',
      };
      data.materiais.push(item);
      data.atualizadoEm = new Date().toISOString();
      writeTenant(slug, 'obras-ops-hoje.json', data);
      pushLog(ctx, slug, 'obras.material.criar', item.nome);
      return sendJSON(res, 200, painelObrasOps(slug));
    }
    ...
    if (idx < 0) return sendJSON(res, 404, { error: 'Material não encontrado' });
    if (body.qtd !== undefined) data.materiais[idx].qtd = Math.max(0, Number(body.qtd) || 0);
    if (body.minimo !== undefined) data.materiais[idx].minimo = Number(body.minimo);
    if (body.nome) data.materiais[idx].nome = String(body.nome).trim().slice(0, 80);
    if (body.unidade) data.materiais[idx].unidade = String(body.unidade).trim().slice(0, 12);
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'obras-ops-hoje.json', data);
    pushLog(ctx, slug, 'obras.material', `${data.materiais[idx].nome}=${data.materiais[idx].qtd}`);
    return sendJSON(res, 200, painelObrasOps(slug));
  }

  if (pathname === '/api/obras-ops/checklist' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const body = await readBody(req);
    const data = ensureObrasOps(slug);
    ...
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'obras-ops-hoje.json', data);
    pushLog(ctx, slug, 'obras.checklist', `${data.checklist[idx].label}=${data.checklist[idx].ok}`);
    return sendJSON(res, 200, painelObrasOps(slug));
  }

  if (pathname === '/api/obras-ops/rota' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const body = await readBody(req);
    const data = ensureObrasOps(slug);
    if (body.acao === 'rebuild') {
      ...
      const idx = data.rota.findIndex((r) => r.pontoId === body.pontoId || String(r.ordem) === String(body.ordem));
      if (idx < 0) return sendJSON(res, 404, { error: 'Ponto da rota não encontrado' });
      if (body.status) data.rota[idx].status = body.status;
      if (body.status === 'concluido') {
        const pIdx = data.pontosQuentes.findIndex((p) => p.id === data.rota[idx].pontoId);
        if (pIdx >= 0) data.pontosQuentes[pIdx].status = 'concluido';
      }
      if (body.status === 'em_atendimento') {
        const pIdx = data.pontosQuentes.findIndex((p) => p.id === data.rota[idx].pontoId);
        if (pIdx >= 0) data.pontosQuentes[pIdx].status = 'em_execucao';
      }
    }
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'obras-ops-hoje.json', data);
    pushLog(ctx, slug, 'obras.rota', body.acao || body.status || 'update');
    return sendJSON(res, 200, painelObrasOps(slug));
  }

  if (pathname === '/api/obras-ops/obra' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const body = await readBody(req);
    const obras = readTenant(slug, 'obras.json', []);
    const idx = obras.findIndex((o) => o.id === body.id);
    if (idx < 0) return sendJSON(res, 404, { error: 'Obra não encontrada' });
    if (body.percentual !== undefined) obras[idx].percentual = Math.max(0, Math.min(100, Number(body.percentual)));
    if (body.status) obras[idx].status = body.status;
    if (obras[idx].percentual >= 100) obras[idx].status = 'concluida';
    writeTenant(slug, 'obras.json', obras);
    ensureObrasOps(slug);
    const data = readTenant(slug, 'obras-ops-hoje.json', {});
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'obras-ops-hoje.json', data);
    pushLog(ctx, slug, 'obras.obra', `${obras[idx].id} ${obras[idx].percentual}%`);
    return sendJSON(res, 200, painelObrasOps(slug));
  }
  ...
  if (pathname === '/api/gabinete/decidir' && req.method === 'POST') {
    const slug = requireTenant(req, url, res);
    if (!slug) return true;
    const user = requireStaffTenant(ctx, res, slug, ['admin', 'prefeito', 'platform']);
    if (!user) return true;
  ...
  if (chMatch && chMatch[1] !== undefined) {
    ...
    if (req.method === 'GET' && !action) {
      ...
      if (!isPapelOps(user?.papel)) return sendJSON(res, 200, sanitizeChamadoPublico(c));
    ...
    if (action === 'comentario' && req.method === 'POST') {
      ...
      if (!texto) return sendJSON(res, 400, { error: 'Escreva a mensagem' });
      ...
      if (user.papel === 'secretaria' && user.secretaria && c.secretaria && c.secretaria !== user.secretaria) {
        return sendJSON(res, 403, { error: 'Chamado de outra secretaria' });
      }
      const now = new Date().toISOString();
      const de = body.de || user.papel || 'staff';
      ...
      c.historico.push({
        em: now,
        status: c.status,
        nota: de === 'campo' || user.papel === 'campo'
          ? `Campo → secretaria: ${texto.slice(0, 120)}`
          : `Secretaria → campo: ${texto.slice(0, 120)}`,
        por: user.nome,
      });
      c.atualizadoEm = now;
      list[idx] = c;
      writeTenant(slug, 'chamados.json', list);
      if (user.papel === 'campo' || de === 'campo') {
        pushNotif(ctx, slug, {
          canal: 'email',
          tipo: 'chat_campo',
          titulo: `Campo · ${c.protocolo}`,
          destino: c.secretaria,
          mensagem: texto,
          chamadoId: c.id,
        });
      } else if (user.papel === 'secretaria' || user.papel === 'admin') {
        pushNotif(ctx, slug, {
          canal: 'email',
          tipo: 'chat_secretaria',
          titulo: `Secretaria · ${c.protocolo}`,
          destino: 'campo',
          mensagem: texto,
          chamadoId: c.id,
        });
      }
      pushLog(ctx, slug, 'os.chat', `${c.protocolo} ${user.papel}: ${texto.slice(0, 80)}`);
      return sendJSON(res, 200, c);
    }
    ...
    if (action === 'anexo' && req.method === 'POST') {
      const user = requireStaffTenant(ctx, res, slug, ['admin', 'prefeito', 'secretaria', 'campo', 'platform']);
      if (!user) return true;
      const body = await readBody(req);
      const list = readTenant(slug, 'chamados.json', []);
      const idx = list.findIndex(x => x.id === id || x.protocolo === id);
    ...
    if (action === 'os' && req.method === 'POST') {
      ...
      } else if (acao === 'deslocamento') {
        c.ordemServico = { ...(c.ordemServico || {}), status: 'em_deslocamento', saidaEm: now };
        c.status = 'em_execucao';
        c.historico.push({ em: now, status: 'em_execucao', nota: body.nota || 'Equipe em deslocamento até o local', por: user.nome });
      } else if (acao === 'checkin') {
        c.ordemServico = {
          ...(c.ordemServico || {}),
          status: 'no_local',
          checkinEm: now,
          lat: body.lat != null ? Number(body.lat) : (c.ordemServico?.lat ?? null),
          lng: body.lng != null ? Number(body.lng) : (c.ordemServico?.lng ?? null),
        };
        c.status = 'em_execucao';
        c.historico.push({ em: now, status: 'em_execucao', nota: body.nota || 'Check-in no local — serviço iniciado', por: user.nome });
      } else if (acao === 'aguardar_material') {
        c.status = 'aguardando_material';
        c.ordemServico = { ...(c.ordemServico || {}), status: 'aguardando_material', materialPedido: body.material || null };
        c.historico.push({
          em: now,
          status: 'aguardando_material',
          nota: body.nota || (body.material ? `Aguardando material: ${body.material}` : 'Aguardando material na base'),
          por: user.nome,
        });
        pushNotif(ctx, slug, {
          canal: 'email',
          tipo: 'material_campo',
          titulo: `Material · ${c.protocolo}`,
          destino: c.secretaria,
          mensagem: body.nota || `Campo pediu material${body.material ? `: ${body.material}` : ''}.`,
          chamadoId: c.id,
        });
      } else if (acao === 'retomar') {
        c.status = 'em_execucao';
        c.ordemServico = { ...(c.ordemServico || {}), status: 'no_local' };
        c.historico.push({ em: now, status: 'em_execucao', nota: body.nota || 'Serviço retomado no local', por: user.nome });
      } else if (acao === 'ocorrencia') {
        const tipo = String(body.tipo || 'outro').slice(0, 40);
        const texto = String(body.texto || body.nota || '').trim().slice(0, 400);
        c.ordemServico = { ...(c.ordemServico || {}), ultimaOcorrencia: { tipo, texto, em: now } };
        c.historico.push({
          em: now,
          status: c.status,
          nota: texto ? `Ocorrência (${tipo}): ${texto}` : `Ocorrência registrada: ${tipo}`,
          por: user.nome,
        });
        if (body.avisarSecretaria) {
          pushNotif(ctx, slug, {
            canal: 'email',
            tipo: 'ocorrencia_campo',
            titulo: `Ocorrência · ${c.protocolo}`,
            destino: c.secretaria,
            mensagem: texto || tipo,
            chamadoId: c.id,
          });
        }
      } else if (acao === 'checklist') {
        const chk = {
          epi: !!body.epi,
          sinalizacao: !!body.sinalizacao,
          transito: !!body.transito,
          fotoAntes: !!body.fotoAntes,
          em: now,
        };
        c.ordemServico = { ...(c.ordemServico || {}), checklistCampo: chk };
        c.historico.push({
          em: now,
          status: c.status,
          nota: `Checklist campo: EPI ${chk.epi ? 'ok' : '—'} · sinalização ${chk.sinalizacao ? 'ok' : '—'} · trânsito ${chk.transito ? 'ok' : '—'}`,
          por: user.nome,
        });
      } else if (acao === 'pessoas') {
        const n = Math.max(1, Math.min(20, Number(body.pessoas) || 1));
        c.ordemServico = { ...(c.ordemServico || {}), pessoas: n };
        c.historico.push({ em: now, status: c.status, nota: `${n} pessoa(s) no serviço`, por: user.nome });
      } else if (acao === 'obs') {
        const texto = String(body.texto || body.nota || '').trim().slice(0, 500);
        if (!texto) return sendJSON(res, 400, { error: 'Informe a observação' });
        c.ordemServico = { ...(c.ordemServico || {}), obsCampo: texto, obsEm: now };
        c.historico.push({ em: now, status: c.status, nota: `Obs. campo: ${texto}`, por: user.nome });
      } else if (acao === 'reforco') {
        const motivo = String(body.motivo || body.nota || 'Precisa reforço de equipe/frota').slice(0, 300);
        c.ordemServico = { ...(c.ordemServico || {}), reforcoPedidoEm: now, reforcoMotivo: motivo };
        c.historico.push({ em: now, status: c.status, nota: `Pedido de reforço: ${motivo}`, por: user.nome });
        pushNotif(ctx, slug, {
          canal: 'email',
          tipo: 'reforco_campo',
          titulo: `Reforço · ${c.protocolo}`,
          destino: c.secretaria,
          mensagem: motivo,
          chamadoId: c.id,
        });
