...
async function handleModulos(ctx) {
  ...
  function painelObrasOps(slug) {
    ...
    const chamados = todosChamados.filter((c) => c.secretaria === 'obras' && isPendente(c.status));
    const obras = readTenant(slug, 'obras.json', []);
    const demoBuraco = todosChamados.find((c) => c.demoFixo === 'buraco-foto')
      || chamados.find((c) => /djalma|buraco/i.test(`${c.endereco || ''} ${c.titulo || ''} ${c.descricao || ''}`));
    ...
    return {
      ...
      chamados: listaChamados.slice(0, 12),
    };
  }
