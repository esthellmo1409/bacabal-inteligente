...
async function handleModulos(ctx) {
  ...
  function ensureObrasOps(slug) {
    ...
    if (data && data.seedVersion === 2) return data;
    if (!data || data.seedVersion !== 2) {
      ...
      data = {
        ...
        equipes: [
          { id: 'EQ-OBR-01', nome: 'Equipe Asfalto 01', lider: 'José Carlos', status: 'em_campo', veiculoId: 'OBR-VEI-01', pontoId: 'PQ-DJALMA' },
          { id: 'EQ-OBR-02', nome: 'Equipe Calçada', lider: 'Ana Paula', status: 'base', veiculoId: 'OBR-VEI-03', pontoId: 'PQ-CALCADA' },
          { id: 'EQ-OBR-03', nome: 'Equipe Drenagem', lider: 'Roberto Lima', status: 'disponivel', veiculoId: null, pontoId: null },
        ],
        frota: [
  ...
  if (pathname === '/api/obras-ops/equipe' && req.method === 'POST') {
    ...
    if (body.pontoId !== undefined) data.equipes[idx].pontoId = body.pontoId || null;
    if (body.veiculoId !== undefined) data.equipes[idx].veiculoId = body.veiculoId || null;
    data.atualizadoEm = new Date().toISOString();
    writeTenant(slug, 'obras-ops-hoje.json', data);
    pushLog(ctx, slug, 'obras.equipe', `${data.equipes[idx].id} ${data.equipes[idx].status}`);
    return sendJSON(res, 200, painelObrasOps(slug));
  }
