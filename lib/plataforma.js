/**
 * CidadeHub / Bacabal Conecta — núcleo da plataforma modular
 */
const STATUS = [
  ...
  { id: 'aguardando_aprovacao', label: 'Aguardando aprovação', cor: '#0ea5e9', pendente: true },
...
const EIXOS = [
  { id: 'cidade', nome: 'Cidade', desc: 'Limpeza, trânsito e serviços urbanos' },
  { id: 'obras', nome: 'Obras', desc: 'Pontos quentes, equipes, frota e materiais' },
  { id: 'iluminacao', nome: 'Iluminação', desc: 'Postes, rota, frota e estoque LED' },
...
const EIXO_SECRETARIAS = {
  cidade: ['limpeza', 'meio_ambiente', 'transito', 'defesa_civil', 'ouvidoria'],
  obras: ['obras'],
...
function normalizeChamado(c) {
  ...
  let fotosAntes = Array.isArray(c.fotosAntes) ? c.fotosAntes.filter(Boolean) : [];
  let fotosDepois = Array.isArray(c.fotosDepois) ? c.fotosDepois.filter(Boolean) : [];

  // Compat: campos únicos + anexos antigos
  if (!fotosAntes.length) {
    const capa = c.fotoAntes || c.foto || null;
    if (capa) fotosAntes = [capa];
  }
  if (!fotosDepois.length && c.fotoDepois) {
    fotosDepois = [c.fotoDepois];
  }
  if (Array.isArray(c.anexos)) {
    for (const a of c.anexos) {
      const url = a && (a.url || a.foto);
      if (!url) continue;
      if ((a.tipo === 'foto' || a.tipo === 'antes') && !fotosAntes.includes(url)) {
        fotosAntes.push(url);
      }
      if ((a.tipo === 'foto_depois' || a.tipo === 'depois') && !fotosDepois.includes(url)) {
        fotosDepois.push(url);
      }
    }
  }

  fotosAntes = fotosAntes.slice(0, MAX_FOTOS);
  fotosDepois = fotosDepois.slice(0, MAX_FOTOS);
  const fotoAntes = fotosAntes[0] || null;
  ...
  return {
    ...
    anexos: c.anexos || (fotoAntes ? [{ tipo: 'foto', url: fotoAntes, em: c.criadoEm }] : []),
...
/** Remove dados operacionais internos — cidadão/público. */
function sanitizeChamadoPublico(c) {
  if (!c) return c;
  const {
    materiais,
    execucao,
  ...
  if (Array.isArray(comentarios)) {
    publico.comentarios = comentarios.filter((x) => x && x.interno !== true);
  }
  return publico;
}

function isPapelOps(papel) {
  return ['secretaria', 'admin', 'prefeito', 'platform', 'campo', 'equipe'].includes(papel);
}
