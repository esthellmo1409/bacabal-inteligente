...
function assistenteGabinete({
  ...
  const cidadeFila = pend.filter((c) => c.secretaria !== 'saude' && c.secretaria !== 'eventos');
  ...
  if (opcao === 'plataforma' || opcao === 'geral') {
    ...
    return {
      ...
      reply:
        ...
        `Por eixo: Cidade ${cidadeFila.length} · Saúde ${saude.length} · Eventos ${eventos.length}.\n` +
        ...
        `Leitura: a plataforma mostra onde a gestão precisa olhar primeiro — use as abas Cidade, Saúde e Eventos para decidir, priorizar e cobrar.`,
    };
  }
  ...
  return {
    ...
    reply:
      `${tratamento}, posso falar sobre a cidade hoje, projetos/obras, reuniões/agenda, prioridades ou saúde. Escolha uma opção.`,
  };
}
