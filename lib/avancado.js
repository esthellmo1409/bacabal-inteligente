...
function assistenteGabinete({
  ...
  const cidadeFila = pend.filter((c) =>
    !['saude', 'eventos', 'iluminacao'].includes(c.secretaria)
  );
  ...
  if (opcao === 'plataforma' || opcao === 'geral') {
    ...
    return {
      ...
      reply:
        ...
        `Por eixo: Cidade ${cidadeFila.length} · Iluminação ${iluminacao.length} · Saúde ${saude.length} · Eventos ${eventos.length}.\n` +
        `Obras: ${obrasOk.length} concluída(s), ${obrasAndamento.length} em andamento, ${obrasAtrasadas.length} atrasada(s).\n` +
        `Bairros com mais demanda: ${bairrosRank}.\n\n` +
        `Leitura: a plataforma mostra onde a gestão precisa olhar primeiro — use as abas Cidade, Iluminação, Saúde e Eventos para decidir, priorizar e cobrar.`,
    };
  }
  ...
  if (opcao === 'cidade') {
    return {
      ...
      reply:
        ...
        `Na cidade (ruas/obras/limpeza): ${cidadeFila.length} abertas, sendo ${criticos.filter((c) => c.secretaria !== 'saude' && c.secretaria !== 'eventos').length} críticas. ` +
  ...
  if (opcao === 'projetos') {
    return {
      ...
      reply:
        ...
        `Nas ocorrências de cidade há ${cidadeFila.length} chamado(s) ainda abertos ligados a serviços urbanos.`,
    };
  }
  ...
  return {
    ...
    reply:
      `${tratamento}, posso falar sobre a cidade hoje, projetos/obras, reuniões/agenda, prioridades, saúde ou iluminação. Escolha uma opção.`,
  };
}
